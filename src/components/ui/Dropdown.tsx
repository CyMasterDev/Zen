...
export function Dropdown({ options, value, onChange, placeholder = 'Select an option', className }: DropdownProps) {
  ...
  return (
    <div ref={dropdownRef} className={cn("dropdown", className)}>
      <div 
        className="flex items-center justify-between h-12 px-4 bg-white/5 rounded-xl cursor-pointer"
        onClick={() => setIsOpen(!isOpen)}
      >
        <div className="flex items-center gap-2">
          {selectedOption?.icon}
          <span className="text-light-1">{selectedOption?.label || placeholder}</span>
        </div>
